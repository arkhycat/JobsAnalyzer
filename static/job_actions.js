...
function updateJobDetails(job) {
    ...
    html += '<button class="job-button" onclick="markAsBadLocation(' + job.id + ')">Bad location</button>';
    html += '<button class="job-button" onclick="markAsInteresting(' + job.id + ')">Interested</button>';
    ...
    html += '<p class="job-detail">' + job.date_posted + '</p>';
    html += '<p class="job-description">' + job.description + '</p>';
...
function markAsInteresting(jobId) {
    console.log('Marking job as interesting: ' + jobId)
    fetch('/mark_interesting/' + jobId, { method: 'POST' })
        .then(response => response.json())
        .then(data => {
            console.log(data);  // Log the response
            if (data.success) {
                var jobCard = document.querySelector(`.job-item[data-job-id="${jobId}"]`);
                jobCard.classList.add('job-item-interesting');
            }
        });
}
...
function markAsBadLocation(jobId) {
    fetch('/mark_bad_location/' + jobId, { method: 'POST' })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                var jobCard = document.querySelector(`.job-item[data-job-id="${jobId}"]`);
