...
@app.route('/mark_bad_location/<int:job_id>', methods=['POST'])
def mark_bad_location(job_id):
    conn = sqlite3.connect(config["db_path"])
    cursor = conn.cursor()
    cursor.execute("UPDATE jobs SET hidden = 1, bad_location = 1 WHERE id = ?", (job_id,))
    conn.commit()
    conn.close()
    return jsonify({"success": "Job marked as hidden and having a bad location"}), 200

@app.route('/mark_interesting/<int:job_id>', methods=['POST'])
def mark_interesting(job_id):
    print("Interesting clicked!")
    conn = sqlite3.connect(config["db_path"])
    cursor = conn.cursor()
    query = "UPDATE jobs SET interesting = 1 WHERE id = ?"
    print(f'Executing query: {query} with job_id: {job_id}')  # Log the query
    cursor.execute(query, (job_id,))
    conn.commit()
    conn.close()
    return jsonify({"success": "Job marked as interesting"}), 200
...
def get_resume(job_id):
    ...
    conn = sqlite3.connect(config["db_path"])
    cursor = conn.cursor()
    cursor.execute("SELECT description, title, company FROM jobs WHERE id = ?", (job_id,))
    job_tuple = cursor.fetchone()
    if job_tuple is not None:
        # Get the column names from the cursor description
        column_names = [column[0] for column in cursor.description]
